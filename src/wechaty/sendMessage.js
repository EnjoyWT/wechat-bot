...
import { botName, roomWhiteList, aliasWhiteList, adminList } from '../../config.js'
import {executeFastlane as fastlane, spawnFastlane} from "../fastlane/index.js"
import {formateMessage} from "../fastlane/messageHandle.js"
...
export async function defaultMessage(msg, bot) {
  ...
  const isAdmin = adminList.includes(name) //是否是管理员,只有管理员才会进行执行命令
  ...
  if (isText && !isBotSelf) {
    ...
    try {
      ...
      if (isAlias && !room) {
        ...
        var message = formateMessage(content)
        var cmd = message.cmd
        var path = message.path
        console.log(cmd)
        console.log(path)

        if (cmd != null, path != null) { //打包命令
          // await fastlane('resign_lane device_udid:00008101-001A35841A98001E', '/Users/tz/Desktop/Tt').then(function (url) {
          await fastlane(cmd, path).then(function (url) {
            contact.say(url)
        }).catch(error =>{
          contact.say(error)
        })
